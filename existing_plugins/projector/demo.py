...
class TensorBoardProjector:
    ...
    @staticmethod
    def register_embedding(log_dir: str, metadata_path: str, tensor_name: str):
    ...
    @staticmethod
    def save_labels_tsv(log_dir: str, metadata_path: str, labels: np.ndarray):
        with open(os.path.join(log_dir, metadata_path), "w") as f:
            for label in labels:
                f.write('{}\n'.format(label))

    def run(self, embedding: pd.DataFrame, labels: np.ndarray):
